...
fmatter = logging.Formatter('%(asctime)s:%(name)s:%(message)s')

file_handler=logging.FileHandler('logfile.log')
file_handler.setLevel(logging.ERROR)
file_handler.setFormatter(fmatter)

stream_handler= logging.StreamHandler()
stream_handler.setFormatter(fmatter)
...
def tcpPortRange():
    ...
    if tcpResponse == None:
        logger.info("The Port...." + str(dst_port) + "....is filtered and silently dropped.")
        logger.info(tcpResponse)

    elif (tcpResponse.haslayer(TCP)):
            if(tcpResponse.getlayer(TCP).flags == 0x12):
                logger.info("The Port...." + str(dst_port) + "....is open. Sending RST packet to close.")
                send_rst = sr(IP(dst=host_IP)/TCP(sport=srcPort,dport=dst_port,flags="R"),timeout=10)
                logger.info(tcpResponse)
            elif (tcpResponse.getlayer(TCP).flags == 0x14):
                logger.info("The Port...." + str(dst_port) + "....is closed.")
                logger.info(tcpResponse)

            else:
                logger.error('No tcp Response')
                main_menu()
